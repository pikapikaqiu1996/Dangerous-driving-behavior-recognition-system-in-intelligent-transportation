...
MotorSpeed::MotorSpeed(QWidget *parent) : QWidget(parent)
{
    foreground = Qt::green;
    ScaleMajor = 10;
    ScaleMinor = 10;
...
void MotorSpeed::drawCrown(QPainter* painter)
{
    painter->save();
    int radius =100;
...
void MotorSpeed::drawBackgroud(QPainter* painter)
{
    painter->save();
...
void MotorSpeed::drawScale(QPainter* painter)
{
    painter->save();
...
void MotorSpeed::drawScaleNum(QPainter* painter)
{
    painter->save();
    painter->setPen(foreground);
    ...
    for(int i = 0; i <= ScaleMajor; i++)
    {
        ...
        tmpVal = 0.1 * i * ((maxValue - minValue) / ScaleMajor) + minValue;
...
void MotorSpeed::drawIndicator(QPainter* painter)
{
    painter->save();
    QPolygon pts;
    pts.setPoints(3, -3, 0, 3, 0, 0, 60);
    painter->rotate(StartAngel);
    double degRotate = (360.0 - StartAngel - EndAngel) / (10 - minValue)*((value/1000) - minValue);
    //indicator
...
void MotorSpeed::drawTitle(QPainter* painter)
{
    painter->save();
    painter->setPen(foreground);
...
void MotorSpeed::drawNumericValue(QPainter* painter)
{
    //painter->save();
...
void MotorSpeed::paintEvent(QPaintEvent *)
{
...
void MotorSpeed::UpdateAngle()
{
    static int i = 0;
    if(i > 90)
        i = 0;
    i+=1;
    value = 8000*sin(i*3.14/180);
    this->update();
}
